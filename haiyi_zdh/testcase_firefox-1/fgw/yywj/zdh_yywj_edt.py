...
class zdh_yywj_edt(unittest.TestCase):
    def setUp(self):
        login.login_admin(self)
        self.verificationErrors = []
        self.accept_next_alert = True
        now = time.strftime("%Y-%m-%d-%H_%M_%S", time.localtime())
        self.text='zdh_yywj_edt'
        self.edt_text='edt'+now
        self.img_text='img_yywj_edt_error'
    
    def test_zdh_yywj_edt(self):
        u"""修改医院文件"""
        driver = self.driver
        ...
        #添加信息
        edt_text='yyej_edt'+time.strftime("%Y%m%d%H%M%S", time.localtime())
        time.sleep(1)
        allhandles=driver.window_handles
        driver.switch_to_window(allhandles[1])
        #获取属性值（除标题和内容）
        fbwjlx=driver.find_element_by_xpath(".//*[@id='Control_abfde4c9_a02a_4254_a88b_1d3e06edcf67']")
        fbwjlx_selcteds=fbwjlx.find_elements_by_tag_name("option")
        for fbwjlx_selcted in fbwjlx_selcteds:
            if fbwjlx_selcted.get_attribute('selected'):
                fbwjlx_add_text=fbwjlx_selcted.text
        ...
        driver.find_element_by_xpath(".//*[@id='Control_60a7e30c_ef53_4999_a8fa_6b056075d481']").send_keys(edt_text)
        ...
        try:
            ...
            for fbwjlx_selcted in fbwjlx_selcteds:
                if fbwjlx_selcted.get_attribute('selected'):
                    fbwjlx_text=fbwjlx_selcted.text
            self.assertEqual(fbwjlx_text,fbwjlx_add_text )
        except AssertionError as e:
            self.verificationErrors.append(str(e))
            print u'发布文件类型名称为“'+text+'"与添加时的内容"'+edt_text+'"不一致'
            ImageGrab.grab().save(os.getcwd().split('testcase_firefox')[0]+'\\img\\'+self.img_text+'2.png')
            print u'【错误截图】：'+self.img_text+'2.png'
        try:
        ...
        driver.find_element_by_xpath(".//*[@id='CustomrichTextBox']").send_keys(edt2_text)
        driver.find_element_by_xpath(".//*[@id='ibtSubmit']").click()
        alert = driver.switch_to_alert()
        text=alert.text
        alert.accept()
        ...
        driver.switch_to_window(allhandles[1])
        driver.find_element_by_xpath(".//*[@id='Control_60a7e30c_ef53_4999_a8fa_6b056075d481']").clear()
        driver.find_element_by_xpath(".//*[@id='Control_60a7e30c_ef53_4999_a8fa_6b056075d481']").send_keys(edt2_text)
        driver.find_element_by_xpath(".//*[@id='ibtSubmit']").click()
        time.sleep(1)
        try:
