...
class zdh_glgd_del(unittest.TestCase):
    def setUp(self):
        login.login_admin(self)
        self.verificationErrors = []
        self.accept_next_alert = True
        now = time.strftime("%Y%m%d%H%M%S", time.localtime())
        self.del_text='glgd_del'+now
        self.img_text='img_glgd_del_error'
    
    def test_zdh_glgd_del(self):
        u"""删除管理规定"""
        driver = self.driver
        ...
        driver.find_element_by_xpath(".//*[@id='Control_f080c6e8_7f60_4232_a1c9_24f6feeb8c5e']").send_keys(self.del_text)
