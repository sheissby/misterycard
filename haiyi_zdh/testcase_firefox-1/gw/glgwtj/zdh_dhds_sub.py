...
#案例
class zdh_dhds_sub(unittest.TestCase):
    def setUp(self):
        ...
        #定义公文标题+当前时间的函数
        self.sub_text='dhds_sub'+now
        #定义公文收件人、联系人、联系方式、申请背景
        self.sub_text2=u'符晓玲'
        self.sub_text3=u'冯学冠'
        self.sub_text4=u'13398991939'
        self.sub_text13=u'申请电话安装'
        #显示用例名称
        print u'---------------《电话（电视）安装维修申请单提交功能自动化测试》----------------------'
        
    def test_zdh_dhds_sub(self):
        
        #打开公文管理-公文呈报-电话（电视）安装维修申请单-------------------------------------------------------------------
        #打开浏览器
        driver = self.driver
        #定位且鼠标移到公文管理模块
        ...
        time.sleep(1)
        #定位到电话（电视）安装维修申请单的所属框架
        driver.switch_to_frame("diaConIf")  
        #点击电话（电视）安装维修申请单链接
        driver.find_element_by_xpath("html/body/div[2]/div/table[2]/tbody/tr/td[2]/div/ul/li/table/tbody/tr/td[1]/a").click()
        time.sleep(1)   
        ...
        driver.find_element_by_id("1b920990-1c67-4d77-913b-af52b81f0188").clear()
        driver.find_element_by_id("1b920990-1c67-4d77-913b-af52b81f0188").send_keys(self.sub_text4)
        driver.find_element_by_id("274c9e3e-6050-491f-90dc-8e9c9d11e3bb").send_keys(self.sub_text13)
        time.sleep(1)
       
        #收件人选择+++++++++++++++++++++++++++++++++++++
        #点击收件人选择按钮
        driver.find_element_by_xpath(".//*[@id='doc_main']/div[2]/table/tbody/tr[5]/td/img").click()
        #定位到人员选择页面窗口
        allhandles=driver.window_handles
        ...
        self.sub_text6=driver.find_element_by_id("029b94a7-d2bf-4764-89a6-42b5a277e22c").get_attribute('value')
        self.sub_text7=driver.find_element_by_id("31e581c1-97d6-4c3c-9733-5d0d8635e329").get_attribute('value')
...
##        os.system("taskkill /F /IM plugin-container.exe")
        ...
        ddlDetailType.find_element_by_xpath("//option[@value='tb168']").click()
        ...
        try:
            ...
            self.assertEqual(text,u'电话(电视)安装维修申请单')
            print u'公文签批的结果列表里标题“'+self.sub_text+u'”的公文类型是电话(电视)安装维修申请单' 
        except AssertionError as e:
            self.verificationErrors.append(str(e))
            print u'公文签批的结果列表里标题“'+self.sub_text+u'”的公文类型不是电话(电视)安装维修申请单'
        #标题字段判断
        try:
        ...
        #判断申请时间字段
        try:
            text=driver.find_element_by_xpath(".//*[@id='029b94a7-d2bf-4764-89a6-42b5a277e22c']").text
        ...
        #判断申请背景字段
        try:
            text=driver.find_element_by_xpath(".//*[@id='Content']/table/tbody/tr[5]/td/div").text
            self.assertEqual(text, self.sub_text13)
            print u'申请背景字段值“'+text+'"与呈报时的值"'+self.sub_text13+'"一致"'+u'”' 
        except AssertionError as e:
            self.verificationErrors.append(str(e))
            print u'申请背景字段值“'+text+'"与呈报时的值"'+self.sub_text13+'"不一致"'+u'”'
        #判断申请资料附件字段
        try:
