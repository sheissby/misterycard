...
class zdh_cybg_edt(unittest.TestCase):
    def setUp(self):
        login.login_admin(self)
        self.verificationErrors = []
        self.accept_next_alert = True
        now = time.strftime("%Y-%m-%d-%H_%M_%S", time.localtime())
        edt_text='zdh_cybg_edt'
        edt2_text='edt'+now
        self.img_text='img_cybg_edt_error'
    
    def test_zdh_cybg_edt(self):
        u"""修改常用表格"""
        driver = self.driver
        allhandlelist=[]
        ...
        time.sleep(1)
        #添加信息
        edt_text='cybg_edt'+time.strftime("%Y%m%d%H%M%S", time.localtime())
        ...
        fbfw_add_text=driver.find_element_by_id("Page_RightControlId_txbText").get_attribute('value')
        ...
        #完成添加
        #修改信息
        edt2_text='cybg_2edt'+time.strftime("%Y%m%d%H%M%S", time.localtime())
        ...
        driver.find_element_by_xpath(".//*[@id='Control_46d6558c_aa37_428b_9a50_60fd844581b3']").send_keys(edt_text)
        ...
        time.sleep(1)
        try:
            fbfw_text=driver.find_element_by_id("Page_RightControlId_txbText").get_attribute('value')
            self.assertEqual(fbfw_text,fbfw_add_text )
        except AssertionError as e:
            self.verificationErrors.append(str(e))
            print u'发布浏览范围名称为“'+text+'"与添加时的内容"'+edt_text+'"不一致'
        ...
        driver.switch_to_window(allhandlelist[1])
        driver.find_element_by_xpath(".//*[@id='Control_46d6558c_aa37_428b_9a50_60fd844581b3']").clear()
        driver.find_element_by_xpath(".//*[@id='Control_46d6558c_aa37_428b_9a50_60fd844581b3']").send_keys(edt2_text)
        driver.find_element_by_xpath(".//*[@id='ibtSubmit']").click()
        time.sleep(1)
        try:
