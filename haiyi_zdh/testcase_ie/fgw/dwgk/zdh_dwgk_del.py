...
class zdh_dwgk_del(unittest.TestCase):
    def setUp(self):
        login.login_admin(self)
        self.verificationErrors = []
        self.accept_next_alert = True
        now = time.strftime("%Y%m%d%H%M%S", time.localtime())
        self.del_text='dwgk_rel'+now
        self.img_text='img_dwgk_del_error'
    
    def test_zdh_dwgk_del(self):
        u"""删除党务公开"""
        driver = self.driver
        allhandlelist=[]
...
##        driver.switch_to_window(allhandlelist[1])
        ...
        driver.find_element_by_xpath(".//*[@id='Control_605af416_b00c_4db9_9e0e_37f13214890d']").send_keys(self.del_text)
