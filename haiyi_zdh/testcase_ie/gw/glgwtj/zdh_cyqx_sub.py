...
#案例
class zdh_cyqx_sub(unittest.TestCase):
    def setUp(self):
        ...
        #定义公文标题+当前时间的函数
        self.sub_text='cyqx_sub'+now
        #定义公文收件人、申请人、器械名称、单台(套)估价
        ...
        #显示用例名称
        print u' '
        print u'---------------《常用器械采购申请单 提交功能自动化测试》----------------------'
        
    def test_zdh_cyqx_sub(self):
        
        #打开公文管理-公文呈报-常用器械采购申请单 -------------------------------------------------------------------
        #打开浏览器
        driver = self.driver
        #赋窗口空值，获取当前窗口
        allhandlelist=[]
        ...
        #点击常用器械采购申请单 链接
        driver.find_element_by_xpath("html/body/div[2]/div/table[3]/tbody/tr/td[2]/div/ul/li/table/tbody/tr/td[4]/a").click()
        time.sleep(1)   
        ...
        driver.find_element_by_id("bddbbdcf-ac2d-4cd7-89d9-82c0d8fbd0e6").send_keys(self.sub_text4)
        driver.find_element_by_id("35ee2bda-45e3-40b5-824c-5d37708cb608").send_keys(self.sub_text5)

        #收件人选择+++++++++++++++++++++++++++++++++++++
        #双击收件人选择按钮
        double=driver.find_element_by_xpath(".//*[@id='doc_main']/div[2]/table/tbody/tr[11]/td/img")
        ...
        self.sub_text6=driver.find_element_by_id("21dced00-60cb-4475-8adb-819bf40420ef").get_attribute('value')
        self.sub_text7=driver.find_element_by_id("SelectOnlyAccount_txbText10").get_attribute('value')
        self.sub_text8=driver.find_element_by_id("a2c78f22-0130-4a45-ba67-3c08805bdf53").get_attribute('value')
        self.sub_text9=driver.find_element_by_id("080105").get_attribute('value')
        self.sub_text10=driver.find_element_by_id("080102").get_attribute('value')
        self.sub_text11=driver.find_element_by_xpath(".//*[@id='tbFilePost_labFilesLink']/ul/li/a[1]").text
        #将缓急、密级字段的值左边的1|去掉（取从左边数2位后的内容）
        self.sub_text12=self.sub_text9[2:]
...
##        time.sleep(1)
        ...
        ddlDetailType.find_element_by_xpath("//option[@value='tb174']").click()
        time.sleep(1)
        ...
        #查询结果判断---------------------------------------------------------------------------------------------
        print u' '
        print u'公文签批页面的查询结果列表中，字段判断如下：'
        #公文类型字段判断
        try:
            text=driver.find_element_by_xpath(".//*[@id='dgQuery']/tbody/tr[2]/td[2]").text
            self.assertEqual(text,u'常用器械采购申请单')
            print u'公文签批的结果列表里标题“'+self.sub_text+u'”的公文类型是“常用器械采购申请单”' 
        except AssertionError as e:
            self.verificationErrors.append(str(e))
            print u'公文签批的结果列表里标题“'+self.sub_text+u'”的公文类型不是“常用器械采购申请单”'
        #标题字段判断
        try:
        ...
        try:
            ...
            print u'单据编号字段值“'+text+'"与呈报时的值"'+self.sub_text6+'"一致"'+u'”' 
        except AssertionError as e:
            self.verificationErrors.append(str(e))
            print u'单据编号字段值“'+text+'"与呈报时的值"'+self.sub_text6+'"不一致"'+u'”' 
        #判断申请人字段
        try:
        ...
        try:
            ...
            print u'申请科室字段值“'+text+'"与呈报时的值"'+self.sub_text7+'"一致"'+u'”' 
        except AssertionError as e:
            self.verificationErrors.append(str(e))
            print u'申请科室字段值“'+text+'"与呈报时的值"'+self.sub_text7+'"不一致"'+u'”' 
        #判断申请时间字段
        try:
            text=driver.find_element_by_xpath(".//*[@id='Content']/table/tbody/tr[2]/td[3]").text
            self.assertEqual(text, self.sub_text8)
            print u'申请时间字段值“'+text+'"与呈报时的值"'+self.sub_text8+'"一致"'+u'”' 
        except AssertionError as e:
            self.verificationErrors.append(str(e))
            print u'申请时间字段值“'+text+'"与呈报时的值"'+self.sub_text8+'"不一致"'+u'”'   
        #判断器械名称字段
        try:
            text=driver.find_element_by_xpath(".//*[@id='Content']/table/tbody/tr[3]/td").text
            self.assertEqual(text, self.sub_text4)
            print u'器械名称字段值“'+text+'"与呈报时的值"'+self.sub_text4+'"一致"'+u'”' 
        except AssertionError as e:
            self.verificationErrors.append(str(e))
            print u'器械名称字段值“'+text+'"与呈报时的值"'+self.sub_text4+'"不一致"'+u'”' 
        #判断单台(套)估价字段
        try:
        ...
        #判断缓急字段
        try:
            text=driver.find_element_by_xpath(".//*[@id='Content']/table/tbody/tr[11]/td[1]/span").text
            self.assertEqual(text, self.sub_text12)
            print u'缓急字段值“'+text+'"与呈报时的值"'+self.sub_text12+'"一致"'+u'”' 
        except AssertionError as e:
            self.verificationErrors.append(str(e))
            print u'缓急字段值“'+text+'"与呈报时的值"'+self.sub_text12+'"不一致"'+u'”'
        #判断密级字段
        try:
            text=driver.find_element_by_xpath(".//*[@id='Content']/table/tbody/tr[11]/td[2]/span").text
            self.assertEqual(text, self.sub_text13)
            print u'密级字段值“'+text+'"与呈报时的值"'+self.sub_text13+'"一致"'+u'”' 
        except AssertionError as e:
            self.verificationErrors.append(str(e))
            print u'密级字段值“'+text+'"与呈报时的值"'+self.sub_text13+'"不一致"'+u'”'
        #判断正文附件字段
        try:
            text=driver.find_element_by_xpath(".//*[@id='AttachmentDIV5']/a[2]").text
            self.assertEqual(text, self.sub_text11)
            print u'正文附件“'+text+'"与添加时的"'+self.sub_text11+'"一致"'+u'”' 
        except AssertionError as e:
            self.verificationErrors.append(str(e))
            print u'正文附件“'+text+'"与添加时的"'+self.sub_text11+'"不一致"'+u'”' 
        time.sleep(1)
