...
#案例
class zdh_yyhc_sub(unittest.TestCase):
    def setUp(self):
        ...
        #定义公文标题+当前时间的函数
        self.sub_text='yyhc_sub'+now
        #定义公文收件人、申请人、医用耗材名称、单价(估)、每月使用量
        self.sub_text2=u'符晓玲'
        self.sub_text3=u'冯学冠'
        self.sub_text4=u'安卓显示器'
        self.sub_text5=u'100'
        self.sub_text6=u'30'
     
        #显示用例名称
        print u'---------------《医用耗材采购申请单 提交功能自动化测试》----------------------'
        
    def test_zdh_yyhc_sub(self):
        
        #打开公文管理-公文呈报-医用耗材采购申请单 -------------------------------------------------------------------
        #打开浏览器
        driver = self.driver
        #赋窗口空值，获取当前窗口
        allhandlelist=[]
        ...
        #点击医用耗材采购申请单 链接
        driver.find_element_by_xpath("html/body/div[2]/div/table[3]/tbody/tr/td[2]/div/ul/li/table/tbody/tr/td[2]/a").click()
        time.sleep(1)   
        ...
        driver.find_element_by_id("9b4e9169-2350-4443-992a-cb76b2e91433").send_keys(self.sub_text4)
        driver.find_element_by_id("40586e1d-8fcd-4cf5-bd2f-f392ebb37423").send_keys(self.sub_text5)
        driver.find_element_by_id("6012705c-9761-42d1-a714-d92f30b13cf8").send_keys(self.sub_text6)
        time.sleep(1)
        
        #收件人选择+++++++++++++++++++++++++++++++++++++
        #双击收件人选择按钮
        double=driver.find_element_by_xpath(".//*[@id='doc_main']/div[2]/table/tbody/tr[18]/td/img")
        ...
        self.sub_text7=driver.find_element_by_id("a270ea3d-3f15-4106-adf3-9e245c7e3b64").get_attribute('value')
        self.sub_text8=driver.find_element_by_id("SelectOnlyAccount_txbText8").get_attribute('value')
        self.sub_text9=driver.find_element_by_id("09e74140-09e1-41d7-9c72-094ac5de4374").get_attribute('value')
...
##        time.sleep(1)
        ...
        ddlDetailType.find_element_by_xpath("//option[@value='tb172']").click()
        time.sleep(1)
        ...
        try:
            ...
            self.assertEqual(text,u'医用耗材采购申请单')
            print u'公文签批的结果列表里标题“'+self.sub_text+u'”的公文类型是“医用耗材采购申请单”' 
        except AssertionError as e:
            self.verificationErrors.append(str(e))
            print u'公文签批的结果列表里标题“'+self.sub_text+u'”的公文类型不是“医用耗材采购申请单”'
        #标题字段判断
        try:
        ...
        try:
            ...
            print u'申请科室字段值“'+text+'"与呈报时的值"'+self.sub_text8+'"一致"'+u'”' 
        except AssertionError as e:
            self.verificationErrors.append(str(e))
            print u'申请科室字段值“'+text+'"与呈报时的值"'+self.sub_text8+'"不一致"'+u'”' 
        #判断申请时间字段
        try:
            text=driver.find_element_by_xpath(".//*[@id='Content']/table/tbody/tr[2]/td[3]").text
            self.assertEqual(text, self.sub_text9)
            print u'申请时间字段值“'+text+'"与呈报时的值"'+self.sub_text9+'"一致"'+u'”' 
        except AssertionError as e:
            self.verificationErrors.append(str(e))
            print u'申请时间字段值“'+text+'"与呈报时的值"'+self.sub_text9+'"不一致"'+u'”'  
        #判断医用耗材名称字段
        try:
            text=driver.find_element_by_xpath(".//*[@id='Content']/table/tbody/tr[3]/td").text
            self.assertEqual(text, self.sub_text4)
            print u'医用耗材名称字段值“'+text+'"与呈报时的值"'+self.sub_text4+'"一致"'+u'”' 
        except AssertionError as e:
            self.verificationErrors.append(str(e))
            print u'医用耗材名称字段值“'+text+'"与呈报时的值"'+self.sub_text4+'"不一致"'+u'”' 
        #判断单价(估)字段
        try:
            text=driver.find_element_by_xpath(".//*[@id='Content']/table/tbody/tr[17]/td/label[2]").text
            self.assertEqual(text,self.sub_text5)
            print u'单价(估)字段值“'+text+'"与呈报时的值"'+self.sub_text5+'"一致"'+u'”' 
        except AssertionError as e:
            self.verificationErrors.append(str(e))
            print u'单价(估)字段值“'+text+'"与呈报时的值"'+self.sub_text5+'"不一致"'+u'”' 
        #判断每月使用量字段
        try:
            text=driver.find_element_by_xpath(".//*[@id='Content']/table/tbody/tr[17]/td/label[5]").text
            self.assertEqual(text, self.sub_text6)
            print u'每月使用量字段值“'+text+'"与呈报时的值"'+self.sub_text6+'"一致"'+u'”' 
        except AssertionError as e:
            self.verificationErrors.append(str(e))
            print u'每月使用量字段值“'+text+'"与呈报时的值"'+self.sub_text6+'"不一致"'+u'”' 
        #判断缓急字段
        try:
            text=driver.find_element_by_xpath(".//*[@id='Content']/table/tbody/tr[18]/td[1]/span").text
        ...
        #判断密级字段
        try:
            text=driver.find_element_by_xpath(".//*[@id='Content']/table/tbody/tr[18]/td[2]/span").text
